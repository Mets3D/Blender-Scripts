...
class Reset_Rig_Colors(bpy.types.Operator):
	...
	bl_idname = "object.reset_rig_colors_hail"
	...
	def execute(self, context):
		rig = context.object
		for cp in rig.rig_colorproperties_hail:
			cp.color = cp.default
		return {'FINISHED'}
...
class Rig_BoolProperties(bpy.types.PropertyGroup):
	...
	"""
	# This is currently only used for outfit/character settings, NOT rig settings. Those booleans are instead hard-coded into rig_properties_hail.
	...
	def update_id_prop(self, context):
		...
		rig = get_rig()
		rig_props = rig.rig_properties_hail
		...
		# Also force an update on the outfit selector - I forget why this is needed, but otherwise the booleans don't work on proxy meshes until the outfit is re-selected
		# Meh, causes infinite loop, ofc.
		#rig.rig_properties_hail.outfit = rig.rig_properties_hail.outfit
...
class Rig_Properties(bpy.types.PropertyGroup):
	...
	def get_rig(self):
		...
		for rig in get_rigs():
			if(rig.rig_properties_hail == self):
				return rig
	...
	def update_bool_properties(self, context):
		...
		rig = self.get_rig()
		bool_props = rig.rig_boolproperties_hail
...
class RigUI_Outfits(RigUI):
	bl_idname = "OBJECT_PT_rig_ui_properties_hail"
	bl_label = "Outfits"
...
class RigUI_Layers(RigUI):
	bl_idname = "OBJECT_PT_rig_ui_layers_hail"
	bl_label = "Layers"
...
class RigUI_Settings_FKIK(RigUI):
	bl_idname = "OBJECT_PT_rig_ui_settings_hail"
	bl_label = "Settings"
...
class RigUI_Settings_FKIK_Switch(RigUI):
	bl_idname = "OBJECT_PT_rig_ui_ikfk_switch_hail"
	bl_label = "FK/IK Switch"
	bl_parent_id = "OBJECT_PT_rig_ui_settings_hail"
...
class RigUI_Settings_IK(RigUI):
	bl_idname = "OBJECT_PT_rig_ui_ik_hail"
...
class RigUI_Settings_FK(RigUI):
	bl_idname = "OBJECT_PT_rig_ui_fk_hail"
...
class RigUI_Settings_Face(RigUI):
	bl_idname = "OBJECT_PT_rig_ui_face_hail"
...
class RigUI_Settings_Misc(RigUI):
	bl_idname = "OBJECT_PT_rig_ui_misc_hail"
	bl_label = "Misc"
	bl_parent_id = "OBJECT_PT_rig_ui_settings_hail"
...
class RigUI_Viewport_Display(RigUI):
	bl_idname = "OBJECT_PT_rig_ui_viewport_display_hail"
...
bpy.types.Object.rig_properties_hail = bpy.props.PointerProperty(type=Rig_Properties)
bpy.types.Object.rig_boolproperties_hail = bpy.props.CollectionProperty(type=Rig_BoolProperties)
bpy.types.Object.rig_colorproperties_hail = bpy.props.CollectionProperty(type=Rig_ColorProperties)
